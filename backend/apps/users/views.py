...
from apps.functions.constants import HTTPCode
from apps.functions.helper import custom_response
...
class Users(Resource):
	def post(self):
		if len(request.json['username'])==0 and len(request.json['password'])==0 and len(request.json['email'])==0:
...
class Login(Resource):
	def post(self):
		...
		if get_user and check_pass:
			...
			return jsonify({'user': '{}'.format(get_user.username),'access_token': access_token,'refresh_token': refresh_token,'data':get_user.data})
